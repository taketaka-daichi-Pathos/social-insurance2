    ...
    if (user) {
        ...
        try {
            ...
            if (userSnap.exists()) {
                ...
                // 💡 ラジオボタン・チェックボックスの復元
                if (data.isForeigner) {
                    const radioForeign = document.getElementById('radio-foreign');
                    if (radioForeign)
                        radioForeign.checked = true;
                }
                if (data.hasPreviousIncome) {
                    const radioIncomeYes = document.getElementById('radio-income-yes');
                    if (radioIncomeYes)
                        radioIncomeYes.checked = true;
                }
                if (data.hasDependent) {
                    const radioDepYes = document.querySelector('input[name="has-dependent"][value="yes"]');
                    if (radioDepYes)
                        radioDepYes.checked = true;
                }
...
// 2. 住所分岐
const addressSameCheck = document.getElementById('address-same-check');
const registeredAddressSection = document.getElementById('registered-address-section');
addressSameCheck?.addEventListener('change', (e) => {
    if (registeredAddressSection) {
        registeredAddressSection.classList.toggle('hidden', e.target.checked);
    }
});
// 3. 前職・所得分岐（はい・いいえの両方を監視）
function toggleTaxSection() {
    const radioIncomeYes = document.getElementById('radio-income-yes');
    const taxSlipSection = document.getElementById('tax-slip-section');
...
function calculateRequiredDocs() {
    ...
    if (status === '直近で退職/失業保険終了')
        requiredDocs.add('退職証明書 または 離職票（１・２）のコピー');
    if (status === '16歳以上の学生')
...
infoForm?.addEventListener('submit', async (e) => {
    ...
    if (!currentUserEmail || !currentUserId)
        return alert("認証エラーです。再ログインしてください。");
    // ==========================================
    ...
    // 📸 2. 画像ファイルの必須バリデーション
    // ==========================================
    const validateImage = (inputId, errorMsg) => {
    ...
    // 💥 画像が足りなければここで送信ストップ！
    if (!validateImage('mynumber-image', '⚠️ マイナンバーの画像が添付されていません！'))
        return;
    if (!validateImage('pension-image', '⚠️ 年金手帳（または基礎年金番号通知書）の画像が添付されていません！'))
        return;
    if (!validateImage('emp-insurance-image', '⚠️ 雇用保険被保険者証の画像が添付されていません！'))
        return;
    // ==========================================
    ...
    try {
        ...
        const getFileInput = (id) => document.getElementById(id);
        const userFolder = `uploads/${currentUserId}`;
        // 1. 本人のドキュメントのアップロード
        let myNumberImageUrl = '';
        const myNumFile = getFileInput('mynumber-image')?.files?.[0];
        if (myNumFile)
            myNumberImageUrl = await uploadFileToStorage(myNumFile, `${userFolder}/mynumber`);
        let pensionImageUrl = '';
        const pensionFile = getFileInput('pension-image')?.files?.[0];
        if (pensionFile)
            pensionImageUrl = await uploadFileToStorage(pensionFile, `${userFolder}/pension`);
        let taxSlipImageUrl = '';
        const radioIncomeYes = document.getElementById('radio-income-yes');
        if (radioIncomeYes?.checked) {
            const taxFile = getFileInput('tax-slip-image')?.files?.[0];
            if (taxFile)
                taxSlipImageUrl = await uploadFileToStorage(taxFile, `${userFolder}/tax_slip`);
        }
        // 🌟 NEW: 雇用保険の画像アップロード処理を追加！
        let empInsuranceImageUrl = '';
        const empInsFile = getFileInput('emp-insurance-image')?.files?.[0];
        if (empInsFile)
            empInsuranceImageUrl = await uploadFileToStorage(empInsFile, `${userFolder}/emp_insurance`);
        // 2. 扶養家族のドキュメントアップロード
        const attachedFiles = [];
        const radioDepYes = document.querySelector('input[name="has-dependent"][value="yes"]');
        if (radioDepYes?.checked) {
        ...
        if (radioDepYes?.checked) {
            dependentData = {
                ...
                hasDisability: document.getElementById('dep-disability')?.checked || false,
        ...
        const employeeData = {
            ...
            registeredAddress: document.getElementById('address-same-check')?.checked ? getValue('current-address') : getValue('registered-address'),
            ...
            willSubmitTaxSlipLater: document.getElementById('tax-slip-later')?.checked || false,
        ...
        if (radioDepYes?.checked && dependentData) {
            try {
                ...
                await addDoc(collection(db, "life_events"), {
                    ...
                    employeeId: employeeData.employeeId || currentUserId.substring(0, 6),
...
export function setupStrictNumberValidation() {
    ...
    document.querySelectorAll('.strict-number').forEach((input) => {
        ...
        if (input.getAttribute('data-strict-bound') === 'true')
            return;
