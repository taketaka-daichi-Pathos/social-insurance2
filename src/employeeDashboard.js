...
async function loadMyPayslips(uid) {
    if (!payslipsListEl)
        return;
    try {
        // 💡 検索条件：「payslips」コレクションの中から、userId が自分のUIDと一致するものだけを探す！
        const q = query(collection(db, 'payslips'), where('userId', '==', uid));
...
// ==========================================
function listenToRemandStatus(email) {
...
async function loadMyNotifications(fullName) {
    const notificationListEl = document.getElementById('notification-list');
    if (!notificationListEl)
        return;
    try {
        const q = query(collection(db, 'notifications'), where('targetEmpName', '==', fullName), orderBy('createdAt', 'desc'));
        ...
        `;
            ...
            document.querySelectorAll('.notif-toggle-area').forEach(area => {
                area.addEventListener('click', (e) => {
                    const detailEl = e.currentTarget.nextElementSibling;
                    if (detailEl)
...
export function initPersonalInfoModal(userEmail) {
    ...
    const btnSave = document.getElementById('btn-save-personal-info');
    const modal = document.getElementById('modal-personal-info');
    const inputPension = document.getElementById('input-emp-pension');
    const inputMyNumber = document.getElementById('input-emp-mynumber');
    const selectHealthType = document.getElementById('select-health-type');
    const inputHealthSymbol = document.getElementById('input-emp-health-symbol');
    const hintHealthSymbol = document.getElementById('hint-health-symbol');
    const inputHealthNumber = document.getElementById('input-emp-health-number');
    if (!btnOpen || !modal || !btnSave || !btnClose)
        return;
    ...
    btnOpen.addEventListener('click', async () => {
        ...
        inputPension.value = '';
        inputMyNumber.value = '';
        if (inputHealthSymbol)
            inputHealthSymbol.value = '';
        if (inputHealthNumber)
            inputHealthNumber.value = '';
        try {
            ...
            if (firstDoc) {
                ...
                if (inputHealthSymbol)
                    inputHealthSymbol.value = userData?.healthInsuranceSymbol || '';
                if (inputHealthNumber)
                    inputHealthNumber.value = userData?.healthInsuranceNumber || '';
...
btnSubmitChange?.addEventListener('click', async () => {
    const changeDate = document.getElementById('input-change-date').value;
    const newLastName = document.getElementById('input-new-lastname').value;
    const newFirstName = document.getElementById('input-new-firstname').value;
    const newZip = document.getElementById('input-new-zip').value;
    const newAddress = document.getElementById('input-new-address').value;
    const newRoute = document.getElementById('input-new-route').value;
    ...
    try {
        ...
        if (modalChange)
            modalChange.style.display = 'none';
        // 入力欄をリセット
        document.getElementById('input-change-date').value = '';
        document.getElementById('input-new-lastname').value = '';
        document.getElementById('input-new-firstname').value = '';
        document.getElementById('input-new-zip').value = '';
        document.getElementById('input-new-address').value = '';
        document.getElementById('input-new-route').value = '';
...
export function initLifeEventForms() {
    ...
    const btnSubmitFamily = document.getElementById('btn-submit-family-event');
    ...
    const calculateRequiredDocs = () => {
        const statusSelect = document.getElementById('dep-status');
        const livingSelect = document.getElementById('dep-living');
        const docNavArea = document.getElementById('doc-nav-area');
        const docUploadContainer = document.getElementById('doc-upload-container');
        if (!statusSelect || !livingSelect || !docNavArea || !docUploadContainer)
            return;
        const status = statusSelect.value;
        const living = livingSelect.value;
        const requiredDocs = new Set();
        // 条件判定（スクショの神ロジック完全再現）
        if (status === '直近で退職/失業保険終了')
            requiredDocs.add('退職証明書 または 離職票（1・2）のコピー');
        if (status === '16歳以上の学生')
            requiredDocs.add('学生証のコピー または 在学証明書');
        if (status === '継続して無職/パート')
            requiredDocs.add('最新の非課税証明書（または課税証明書）');
        if (status === '年金受給者')
            requiredDocs.add('年金振込通知書 および 年金額改定通知書');
        if (living === '別居')
            requiredDocs.add('仕送りしている通帳のコピー（直近の送金実績が分かるもの）');
    ...
    // ==========================================
    // モーダル開閉
    btnApplyFamily?.addEventListener('click', () => { if (modalFamily)
        modalFamily.style.display = 'flex'; });
    const closeModal = () => { if (modalFamily)
    ...
    radioEventTypes.forEach(radio => {
        radio.addEventListener('change', (e) => {
            ...
            if (formBirthSection)
                formBirthSection.style.display = val === 'birth' ? 'block' : 'none';
            if (formReinstatementSection)
                formReinstatementSection.style.display = val === 'reinstatement' ? 'block' : 'none';
            if ((val === 'marriage' || val === 'other') && formFamilySection)
                formFamilySection.style.display = 'block';
            if ((val === 'marriage' || val === 'other') === false)
                if (formFamilySection)
                    formFamilySection.style.display = 'none';
            if (formFamilyRemoveSection)
                formFamilyRemoveSection.style.display = val === 'remove_family' ? 'block' : 'none';
        });
    });
    ...
    btnSubmitFamily?.addEventListener('click', async () => {
        ...
        const selectedType = document.querySelector('input[name="family-event-type"]:checked')?.value;
        if (!selectedType) {
        ...
        if (selectedType === 'birth') {
            ...
            const leaveStart = document.getElementById('input-leave-start')?.value;
            const leaveEnd = document.getElementById('input-leave-end')?.value;
        ...
        try {
            ...
            if (selectedType === 'birth') {
                const birthDate = document.getElementById('input-birth-date')?.value;
                if (!birthDate)
                    throw new Error("出産予定日未入力"); // catchブロックへ飛ばす
                ...
                const options = Array.from(checks).map((cb) => cb.value);
            ...
            // 🔙 復職の場合
                ...
                const returnDate = document.getElementById('input-reinstatement-date')?.value;
                if (!returnDate)
                    throw new Error("復職予定日未入力");
                eventData.eventTitle = '🔙 復職（育休の終了）の申請';
                eventData.eventDate = returnDate;
                eventData.childName = document.getElementById('input-child-name')?.value || '';
                eventData.childBirthDate = document.getElementById('input-child-dob')?.value || '';
            }
            // 💍 結婚 または 🏫 その他の家族追加の場合
            else if (selectedType === 'marriage' || selectedType === 'other') {
                eventData.eventTitle = selectedType === 'marriage' ? '💍 結婚・配偶者扶養の申請' : '🏫 家族・扶養追加の申請';
                eventData.dependent = {
                    lastNameKanji: document.getElementById('dep-last-name-kanji')?.value || '',
                    firstNameKanji: document.getElementById('dep-first-name-kanji')?.value || '',
                    lastNameKana: document.getElementById('dep-last-name-kana')?.value || '',
                    firstNameKana: document.getElementById('dep-first-name-kana')?.value || '',
                    birthDate: document.getElementById('dep-birth')?.value || '',
                    relation: document.getElementById('dep-relation')?.value || '',
                    income: document.getElementById('dep-income')?.value || '',
                    livingStatus: document.getElementById('dep-living')?.value || ''
                };
            ...
            // 👋 扶養から外す（喪失）場合
                ...
                const empName = document.getElementById('remove-dep-select')?.value;
                const reason = document.getElementById('remove-dep-reason')?.value;
                const removeDate = document.getElementById('remove-dep-date')?.value;
                const cardReturn = document.getElementById('remove-dep-card-return')?.value;
            ...
            // ==========================================
            const attachedFiles = [];
            const fileInputs = document.querySelectorAll('.dep-file-input');
            for (const input of Array.from(fileInputs)) {
                const docNavArea = input.closest('#doc-nav-area, #remove-doc-nav-area');
            ...
            if (error.message)
                console.log("入力チェックによる中断:", error.message);
            else
                console.error("申請エラー:", error);
        }
        finally {
...
export function initReissueInsuranceForm() {
    ...
    const reissueReason = document.getElementById('reissue-reason');
    ...
    const formReissue = document.getElementById('form-reissue-insurance');
    if (formReissue) {
        formReissue.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const police = document.getElementById('reissue-police').value;
                const memo = document.getElementById('reissue-memo').value;
                ...
                if (modalReissue)
                    modalReissue.style.display = 'none';
                formReissue.reset();
                if (policeSec)
                    policeSec.style.display = 'none';
                if (brokenSec)
...
export function initResignationForm() {
    ...
    const formResignation = document.getElementById('form-resignation');
    if (formResignation) {
        formResignation.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const resignDate = document.getElementById('resignation-date').value;
                const returnStatus = document.getElementById('insurance-return-status').value;
                const unemploymentSlip = document.querySelector('input[name="unemployment-slip"]:checked')?.value;
                // 🌟 修正：コレクションを「changeRequests」にし、companyIdを同梱、statusを'pending'に統一！
                await addDoc(collection(db, "changeRequests"), {
                    companyId: myCompanyId, // 🔥 必須キー！
                    userId: currentUserId,
...
async function loadFamilyMembers(userId) {
    try {
        ...
        const removeSelect = document.getElementById('remove-dep-select');
        if (!container)
            return;
        ...
        if (validDependents.length === 0) {
            ...
            if (removeSelect)
                removeSelect.innerHTML = '<option value="">対象の扶養家族がいません</option>';
            return;
        }
        // 家族リストとプルダウンのHTMLを組み立てる
        let listHtml = '';
        let selectHtml = '<option value="">対象者を選択してください</option>';
        validDependents.forEach((dep) => {
      ...
      `;
        ...
        if (removeSelect)
            removeSelect.innerHTML = selectHtml;
    }
    catch (error) {
...
initResignationForm();
//# sourceMappingURL=employeeDashboard.js.map