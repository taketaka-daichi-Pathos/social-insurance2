...
// =========================================================
function calcEmployeePremium(wage, rate) {
    // 1. まず標準報酬月額と料率を掛け算する
...
async function loadInsuranceMasterTab() {
    ...
    try {
        ...
        // 👇＝＝＝＝ ここから追加！ ＝＝＝＝👇
        const inputNursingRate = document.getElementById('input-nursing-rate');
        const btnResetNursingRate = document.getElementById('btn-reset-nursing-rate');
        // ☝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝☝
        // 組合健保用
        ...
        const loadCurrentSettings = async () => {
            try {
                ...
                if (settingsList.length > 0) {
                    ...
                    if (inputNursingRate)
                        inputNursingRate.value = ((latestData.nursingRate || 0.0162) * 100).toFixed(2);
                }
            }
            catch (e) {
...
async function initMonthlySalaryUI() {
    ...
    const loadMonthlyData = async () => {
        ...
        if (lockCompanyId) {
            ...
            const latestBtnSave = document.getElementById('btn-save-monthly');
            if (!latestBtnSave)
                return;
            if (statusSnap.exists() && statusSnap.data().isConfirmed) {
                window.isCurrentMonthLocked = true;
            ...
            latestBtnSave.onclick = async () => {
                ...
                try {
                    ...
                    individualSaveBtns.forEach(btn => { btn.click(); });
                    ...
                    latestBtnSave.style.pointerEvents = "none";
                    window.isCurrentMonthLocked = true;
                    document.querySelectorAll('.btn-indiv-save').forEach(btn => { btn.style.display = "none"; });
                    document.querySelectorAll('.btn-toggle-adjust').forEach(btn => { btn.style.display = "none"; });
...
`;
                ...
                saveBtn.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        const insPensionAmount = Number(adjustTr.querySelector('.input-ins-pension')?.value) || 0;
                        const insNursingAmount = Number(adjustTr.querySelector('.input-ins-nursing')?.value) || 0;
                        const insSupportAmount = Number(adjustTr.querySelector('.input-ins-support')?.value) || 0; // 🆕 支援金の調整額取得
                        ...
                        const supportEmpSpan = tr.querySelector('.calc-support-emp'); // 🆕 支援金の宛先を取得
                        const totalEmpSpan = tr.querySelector('.calc-total-emp');
                        if (healthSpan)
                            healthSpan.innerText = finalHealth.toLocaleString();
                        if (pensionSpan)
                            pensionSpan.innerText = finalPension.toLocaleString();
                        if (nursingSpan)
                            nursingSpan.innerText = finalNursing.toLocaleString();
                        if (supportEmpSpan)
                            supportEmpSpan.innerText = finalSupportEmp.toLocaleString(); // 🆕 支援金の画面書き換え
    ...
    const btnExportGeppen = document.getElementById('btn-export-egov-geppen');
    if (btnExportGeppen) {
    ...
    if (btnSaveMonthly) {
        ...
        newBtnSaveMonthly.addEventListener('click', async () => {
            ...
            console.log("🔒 現在のロック判定メモ:", window.isCurrentMonthLocked);
        ...
        try {
          ...
          `;
                ...
                const calcBonusPremium = () => {
                    ...
                    if (newCumulativeSpan)
                        newCumulativeSpan.innerHTML = newCumulative.toLocaleString();
                    const newRemainingSpan = tr.querySelector('.calc-new-remaining');
                    if (newRemainingSpan)
                        newRemainingSpan.innerHTML = newRemaining.toLocaleString();
                };
...
async function initSanteiUI() {
    ...
    const yearSelect = document.getElementById('santei-target-year');
...
`;
            ...
            newSaveBtn.addEventListener('click', async () => {
                ...
                if (!confirm(`全員の新等級をデータベースに予約保存しますか？\n\n※この会社は【${timingText}】設定のため、新しい保険料は「${deductionStartMonth}月支給の給与」から自動で適用されます。`))
                    return;
                // ▲▲▲ ここまで ▲▲▲
...
setTimeout(async () => {
    try {
        ...
        if (snap.exists()) {
            ...
            // 🟢 【追加】控除タイミングの要素を取得
            const deductionTimingSelect = document.getElementById('insurance-deduction-timing');
...
document.addEventListener('click', async (e) => {
    ...
    if (target && target.id === 'btn-save-company-pay-rules') {
        ...
        // 🟢 【追加】控除タイミングの値を取得
        const deductionTimingVal = document.getElementById('insurance-deduction-timing')?.value;
        try {
...
async function initZuijiUI() {
    ...
    try {
        ...
        if (targetSelect) {
            Array.from(targetSelect.options).forEach(option => {
                ...
                const unprocessedCount = rawTargets.filter((t) => {
                    const emp = employees.find((e) => e.id === t.realFirebaseId || String(e.employeeId) === String(t.id));
        ...
        employees.forEach((emp) => {
            if (emp.appliedGeppenYear === revisionYear && emp.appliedGeppenMonth === revisionMonth) {
                ...
                const alreadyExists = targets.some((t) => t.id === (emp.employeeId || emp.id) || t.realFirebaseId === emp.id);
                if (!alreadyExists) {
                    // 過去の月を計算（表示用）
                    let startM = revisionMonth - 3;
                    let startY = revisionYear;
                    if (startM <= 0) {
                        startM += 12;
                        startY -= 1;
                    }
                    let endM = revisionMonth - 1;
                    if (endM <= 0) {
                        endM += 12;
                    }
                    targets.push({
        ...
        window.geppenTargets = targets.filter((t) => !t.isApplied);
    ...
    // =========================================================
    const btnApplyRevisions = document.getElementById('btn-apply-revisions');
    if (btnApplyRevisions) {
        btnApplyRevisions.onclick = async () => {
            // 💡 Step 1で window に置いたデータを取得！
            const targetsToApply = window.geppenTargets || [];
...
async function loadInsuranceData() {
    ...
    try {
        ...
        if (isConfirmed) {
            ...
            if (alertBox)
                alertBox.style.display = "none";
            const q = query(collection(db, "monthly_payroll_records"), where("companyId", "==", lockCompanyId), where("year", "==", insYear), where("month", "==", insMonth));
