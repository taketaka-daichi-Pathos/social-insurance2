...
export async function initLifeEventUI() {
    ...
    async function loadUnifiedRequests() {
        ...
        try {
            ...
            usersSnap.forEach(u => {
                ...
                if (data.email)
                    userDict[data.email] = fullName;
                if (data.employeeId)
                    userDict[data.employeeId] = fullName;
            });
            // 🌟 2. 【防壁②】自社の未承認申請（pending）を『すべて』取得！
            // （※従業員側は changeRequests に一本化しました！）
            const qChange = query(collection(db, 'changeRequests'), where("companyId", "==", currentCompanyId), where('status', '==', 'pending'));
            ...
            if (badge)
                badge.innerText = `未承認: ${allRequests.length}件`;
            ...
            allRequests.forEach(req => {
                  ...
                  追加家族: <strong>${dep.lastNameKanji || ''} ${dep.firstNameKanji || ''}</strong> (${dep.relation || '不明'})
                    ...
                    if (req.options && req.options.length > 0) {
                        ...
                        req.options.forEach((opt) => {
                            ...
                            if (opt === 'allowance')
                                label = '💰 産休中のお給料の代わり（出産手当金）を申請';
                            else if (opt === 'sankyu_exemption')
                                label = '🆓 産休中の社会保険料免除の手続き';
                            else if (opt === 'hellowork')
                                label = '🏢 ハローワークからの育休手当（育児休業給付金）を申請';
                            else if (opt === 'exemption')
                                label = '🆓 育休中の社会保険料免除の手続き';
                            else if (opt === 'monthly_change')
                                label = '🛡️ 復帰後の手取りを守る特例（報酬月額変更）を申請';
                            else if (opt === 'pension_special')
                                label = '👴 将来の年金が減らないようにする特例（標準報酬特例）を申請';
              ...
              <div style="font-size:12px; color:#555; display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                ...
                if (req.originalData.attachedFiles && req.originalData.attachedFiles.length > 0) {
                    ...
                    req.originalData.attachedFiles.forEach((file) => {
    ...
    function attachReqEvents() {
        ...
        const rowChecks = document.querySelectorAll('.req-checkbox');
        const btnBulk = document.getElementById('btn-bulk-approve-req');
        ...
        const updateBulkBtn = () => {
            ...
            if (countSpan)
                countSpan.innerText = checkedCount.toString();
            if (btnBulk) {
                if (checkedCount > 0) {
        ...
        const executeApproval = async (targets) => {
            console.log("🚀 承認・タスク化エンジン起動！ 処理件数:", targets.length);
            if (!confirm(`${targets.length}件の申請を承認し、タスクを生成しますか？\n（※住所変更・扶養追加などのマスタは自動更新されます）`))
                return;
            // 👇 🌟 ここに `window.prompt` のコードを追加します！
            const customMessage = window.prompt(`${targets.length}件の申請を承認し、タスクを生成します。\n対象者に送る通知メッセージを入力してください：\n（※空白のままOKを押すと標準の定型文が送信されます）`, "申請いただいた手続きが労務にて承認され、社内処理が完了しました。");
            if (customMessage === null)
                return; // キャンセルなら処理ストップ
            try {
                const currentCompanyId = localStorage.getItem('current_company_id');
                if (!currentCompanyId) {
                        ...
                        if (!empSnapshot.empty) {
                            ...
                            if (empDoc) {
                                ...
                                if (reqData.newLastName)
                                    updateData.lastNameKanji = reqData.newLastName;
                                if (reqData.newFirstName)
                                    updateData.firstNameKanji = reqData.newFirstName;
    ...
    let selectedIds = []; // チェックされた対象者を保存する箱
    const container = document.getElementById('kaigo-alert-container');
    const badge = document.getElementById('alert-badge');
    function renderEventTable() {
        if (badge)
            badge.innerText = `今月の要対応: ${ageEventUsers.length}件`;
        if (!container)
            return;
    ...
    function attachTableEvents() {
        ...
        rowCheckboxes.forEach(cb => {
            cb.addEventListener('change', (e) => {
                ...
                if (target.checked) {
                    if (!selectedIds.includes(target.value))
                ...
                if (checkAll)
                    checkAll.checked = selectedIds.length === rowCheckboxes.length && rowCheckboxes.length > 0;
                updateBulkButton();
            });
        });
        if (btnBulkGen) {
...
function getAgencyByTaskTitle(title) {
    if (title.includes('厚生年金喪失') || title.includes('後期高齢者'))
        return '年金事務所';
